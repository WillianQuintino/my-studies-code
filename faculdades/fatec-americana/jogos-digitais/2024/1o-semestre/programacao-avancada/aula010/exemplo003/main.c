...
void inset_tree
{
    ...
    {
        printf("\n Deseja Alocar em que vertice [1]-Esquerda // [2]-Direita // [3]-Centro");
        scanf("%d", &Path);
        if(Path == 1)
        {
            aloca_esquerda();
...
aloca_esquerda()
{
    ...
    {
        auxilar=auxiliar->back;
    }
    auxiliar->back=corrente;
    printf("\n Alocado com sucesso a Esquerda do nada \n");
    system("pause");
}
...
void Exibir(no *root)
{
    ...
    else
    {
        ...
        if(root->center != NULL) Exibir(root->centro);
    }
}

int Search_tree(no *root)
{
    int codigo;
    int achou;
    achou = 0;
    printf("\n Digita o codigo a buscar");
    scanf("%d", &codigo);
    printf("\n %d", achou);
...
int Achou_Valor(no *root, int key)
{
    if(key == root->codigo)
    {
        printf("%s", root->nome);
